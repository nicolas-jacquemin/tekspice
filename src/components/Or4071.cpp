...
nts::Tristate nts::Components::Or4071::getPinState(size_t outputPinId) {
    switch (outputPinId) {
    case 3:
    case 10:
        return (nts::gates::orGate(getPin(outputPinId - 2).getState(), getPin(outputPinId - 1).getState()));
    ...
    default:
        getPin(outputPinId);
        throw nts::Error("\"4071\" component: pin " + std::to_string(outputPinId) + " is not an output");
    }
}
